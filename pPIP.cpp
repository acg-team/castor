...
void pPIP::_build_MSA(bpp::Node *node, int idx_sb) {
    ...
    MSA_t *MSA_L = &(MSA_.at(idx_sb).at(sonLeftID));
    MSA_t *MSA_R = &(MSA_.at(idx_sb).at(sonRightID));
...
//            TR[m][i][j] = tr_index;
        ...
        for (i = 1; i < h; i++) {
            for (j = 1; j < w; j++) {
                ...
                Log3DM[m][i][j] = computeLK_MXY_local(log_phi_gamma,
                                                                  Log3DM[m - 1][i-1][j-1],
                                                                  Log3DX[m - 1][i-1][j-1],
                                                                  Log3DY[m - 1][i-1][j-1],
                                                                  Log2DM[id1m][id2m]);
