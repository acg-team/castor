...
pPIP::pPIP(bpp::Alphabet *alphabet) {
    ...
    Alphabet_ = alphabet;
    alphabetSize_ = Alphabet_->getSize();
    extendedAlphabetSize_ = Alphabet_->getSize() + 1;
...
void pPIP::_reserve(unsigned long numNodes) {
    ...
    iota_.resize(numNodes);
    beta_.resize(numNodes);
    pr_.resize(numNodes);
    sequenceNames_.resize(numNodes);
...
bool pPIP::isInside(unsigned long x0, unsigned long y0, unsigned long xf, unsigned long yf, unsigned long xt, unsigned long yt) {
...
void pPIP::setIndicesM(unsigned long &up_corner_i, unsigned long &up_corner_j, unsigned long &bot_corner_i,
...
void pPIP::setIndicesX(unsigned long &up_corner_i, unsigned long &up_corner_j, unsigned long &bot_corner_i,
                       unsigned long &bot_corner_j, unsigned long level, unsigned long h, unsigned long w) {
...
void pPIP::setIndicesY(unsigned long &up_corner_i, unsigned long &up_corner_j, unsigned long &bot_corner_i,
                       unsigned long &bot_corner_j, unsigned long level, unsigned long h, unsigned long w) {
...
signed long pPIP::getIndicesM(unsigned long nx, unsigned long ny, unsigned long up_corner_i, unsigned long up_corner_j,
                              unsigned long bot_corner_i, unsigned long bot_corner_j, unsigned long m, unsigned long h, unsigned long w) {

    signed long idx;

    setIndicesM(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, m, h, w);

    if (isInside(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, nx, ny)) {
...
signed long pPIP::getIndicesX(unsigned long nx, unsigned long ny, unsigned long up_corner_i, unsigned long up_corner_j,
                              unsigned long bot_corner_i, unsigned long bot_corner_j, unsigned long m, unsigned long h, unsigned long w) {

    signed long idx;

    setIndicesX(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, m, h, w);
...
signed long pPIP::getIndicesY(unsigned long nx, unsigned long ny, unsigned long up_corner_i, unsigned long up_corner_j,
                              unsigned long bot_corner_i, unsigned long bot_corner_j, unsigned long m, unsigned long h, unsigned long w) {

    signed long idx;

    setIndicesY(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, m, h, w);
...
void pPIP::setIndicesT(unsigned long &up_corner_i, unsigned long &up_corner_j, unsigned long &bot_corner_i,
                       unsigned long &bot_corner_j, unsigned long level, unsigned long h, unsigned long w) {
    ...
    setIndicesX(up_corner_i_x, up_corner_j_x, bot_corner_i_x, bot_corner_j_x, level, h, w);

    setIndicesY(up_corner_i_y, up_corner_j_y, bot_corner_i_y, bot_corner_j_y, level, h, w);
...
void pPIP::resetCorner(unsigned long &up_corner_i, unsigned long &up_corner_j, unsigned long &bot_corner_i,
...
unsigned long pPIP::getIndicesT(unsigned long nx, unsigned long ny, unsigned long up_corner_i, unsigned long up_corner_j,
                                unsigned long bot_corner_i, unsigned long bot_corner_j, unsigned long m, unsigned long h, unsigned long w) {

    setIndicesT(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, m, h, w);

    resetCorner(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, h, w);
...
int pPIP::getIndexOfMax(double m, double x, double y, double epsilon,
...
double pPIP::getMaxOfThree(double a, double b, double c, double epsilon) {
...
std::string pPIP::createGapColumn(unsigned long len) {
...
void pPIP::buildMSA(bpp::Node *node, std::string traceback_path) {
    ...
    std::vector<std::string> *MSA_L = &(MSA_.at(s1ID));
    std::vector<std::string> *MSA_R = &(MSA_.at(s2ID));
...
void pPIP::setMSASequenceNames(bpp::Node *node) {
    ...
    for (int i = 0; i < sequenceNames_.at(s1ID).size(); i++) {
        seqNames.push_back(sequenceNames_.at(s1ID).at(i));
    }

    for (int i = 0; i < sequenceNames_.at(s2ID).size(); i++) {
        seqNames.push_back(sequenceNames_.at(s2ID).at(i));
    }

    sequenceNames_.at(node->getId()) = seqNames;
...
void pPIP::_setLocalTau(bpp::Node *node) {
    bpp::Tree *subtree = tree_->cloneSubtree(node->getId());
    tau_ = subtree->getTotalLength();
}
...
void pPIP::_setAllIotas(UtreeBppUtils::treemap *tm, std::vector<tshlib::VirtualNode *> &listNodes) {
    ...
    for (auto &vnode:listNodes) {
        ...
        if (!node->hasFather()) {
            iota_.at(node->getId()) = (1 / mu_) / T;
        } else {
            iota_.at(node->getId()) = node->getDistanceToFather() / T;
        }
    }
}
...
void pPIP::_computePr(UtreeBppUtils::treemap *tm, std::vector<tshlib::VirtualNode *> &listNodes) {
    ...
    for (auto &vnode:listNodes) {
        ...
        if (!node->hasFather()) {
            ...
            pr_.at(node->getId()) = MatrixBppUtils::Eigen2Matrix(vnode->getPr());
        }
...
bpp::ColMatrix<double> pPIP::go_down(bpp::Node *node, std::string &s, unsigned long &idx) {
    ...
    if (node->isLeaf()) {
        ...
        bpp::MatrixTools::mult(pr_.at(sonLeftID), fvL, PrfvL);
        bpp::MatrixTools::mult(pr_.at(sonRightID), fvR, PrfvR);
...
double pPIP::computeLikelihoodGapDown(bpp::Node *node, std::string &s) {
    ...
    if (node->isLeaf()) {
        ...
        pr = iota_.at(nodeID) - iota_.at(nodeID) * beta_.at(nodeID) + iota_.at(nodeID) * beta_.at(nodeID) * fv0;
        return pr;
    } else {
        idx = 0;
        fv = go_down(node, s, idx);
        ...
        len = ixx;
        sL = s.substr(0, len);
        pL = computeLikelihoodGapDown(sons.at(LEFT), sL);

        std::string sR;
        sR = s.substr(ixx);
        pR = computeLikelihoodGapDown(sons.at(RIGHT), sR);
    }

    return pr + pL + pR;
}

double pPIP::computeLikelihoodDown(bpp::Node *node, std::string &s) {
    ...
    if (node->isLeaf()) {
        ...
        fv = go_down(node, s, idx);
        fv0 = MatrixBppUtils::dotProd(fv, pi_);
        pr = iota_.at(nodeID) * beta_.at(nodeID) * fv0;
        ...
        if (flagR) {
            ...
            len = ixx;
            sL = s.substr(0, len);
            return pr + computeLikelihoodDown(sons.at(LEFT), sL);
        }

        if (flagL) {
            std::string sR;
            sR = s.substr(ixx);
            return pr + computeLikelihoodDown(sons.at(RIGHT), sR);
        }
...
double pPIP::computeLikelihoodGapColumnLocal(bpp::Node *node, std::string &sL, std::string &sR) {
    double fv0;
    double pr;
    bpp::ColMatrix<double> fvL;
    bpp::ColMatrix<double> fvR;
...
double pPIP::computeLikelihood_M_local(double valM,
                                       double valX,
                                       double valY,
                                       bpp::Node *node,
                                       std::string &sL,
    ...
    val = -log((long double) m) + log((long double) nu_) + pr + getMaxOfThree(valM, valX, valY, DBL_EPSILON);

    return val;
}

double pPIP::computeLikelihood_X_local(double valM,
                                       double valX,
                                       double valY,
                                       bpp::Node *node,
                                       std::string &sL,
    ...
    if (it == lkX.end()) {
        ...
        pL = computeLikelihoodDown(sons.at(LEFT), sL);
...
double pPIP::computeLikelihood_Y_local(double valM,
                                       double valX,
                                       double valY,
                                       bpp::Node *node,
    ...
    if (it == lkY.end()) {
        ...
        pR = computeLikelihoodDown(sons.at(RIGHT), sR);
...
void pPIP::DP3D_PIP(bpp::Node *node, UtreeBppUtils::treemap *tm, double gamma_rate, bool local) {
    ...
    unsigned long numLeavesLeft = sequenceNames_.at(s1ID).size();
    unsigned long numLeavesRight = sequenceNames_.at(s2ID).size();

    col_gap_Ls = createGapColumn(numLeavesLeft);
    col_gap_Rs = createGapColumn(numLeavesRight);
    ...
    //***************************************************************************************
    if (local) {
        pc0 = computeLikelihoodGapColumnLocal(node, col_gap_Ls, col_gap_Rs);
    } else {
        /*
    ...
    LogM[0][0] = nu_ * (pc0 - 1.0);
    LogX[0][0] = nu_ * (pc0 - 1.0);
    LogY[0][0] = nu_ * (pc0 - 1.0);
        ...
        if (checkBoundary(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, h, w)) {
            ...
            for (unsigned long i = up_corner_i; i <= bot_corner_i; i++) {
                ...
                for (int j = 0; j <= lw; j++) {
                    ...
                    idx = getIndicesM(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i, up_corner_j,
                    ...
                    idx = getIndicesX(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i, up_corner_j,
                                      bot_corner_i, bot_corner_j, m - 1, h, w);
                    if (idx >= 0) {
                    ...
                    idx = getIndicesY(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i, up_corner_j,
                                      bot_corner_i, bot_corner_j, m - 1, h, w);
                    if (idx >= 0) {
                    ...
                    if (local) {
                        val = computeLikelihood_M_local(valM,
                                                        valX,
                                                        valY,
                                                        node,
                                                        sLs,
                    ...
                    idx = getIndicesM(coordTriangle_this_i, coordTriangle_this_j, up_corner_i,
        ...
        if (checkBoundary(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, h, w)) {
            ...
            for (unsigned long i = up_corner_i; i <= bot_corner_i; i++) {
                ...
                for (int j = 0; j <= lw; j++) {
                    ...
                    idx = getIndicesM(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i,
                    ...
                    idx = getIndicesX(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i,
                                      up_corner_j, bot_corner_i, bot_corner_j, m - 1, h, w);
                    if (idx >= 0) {
                        valX = LogX[m_binary_prev][idx];
                    } else {
                    ...
                    idx = getIndicesY(coordTriangle_prev_i, coordTriangle_prev_j, up_corner_i,
                                      up_corner_j, bot_corner_i, bot_corner_j, m - 1, h, w);
                    if (idx >= 0) {
                        valY = LogY[m_binary_prev][idx];
                    } else {
                    ...
                    if (local) {
                        val = computeLikelihood_X_local(valM,
                                                        valX,
                                                        valY,
                                                        node,
                                                        sLs, col_gap_Rs,
                    ...
                    idx = getIndicesX(coordTriangle_this_i, coordTriangle_this_j, up_corner_i,
                                      up_corner_j, bot_corner_i, bot_corner_j, m, h, w);
        ...
        if (checkBoundary(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, h, w)) {
            ...
            for (unsigned long i = up_corner_i; i <= bot_corner_i; i++) {
                ...
                for (int j = 0; j <= lw; j++) {
                    ...
                    if (local) {
                        val = computeLikelihood_Y_local(valM,
                                                        valX,
                                                        valY,
                                                        node,
                                                        col_gap_Ls, sRs,
                    ...
                    idx = getIndicesY(coordTriangle_this_i, coordTriangle_this_j, up_corner_i,
                                      up_corner_j, bot_corner_i, bot_corner_j, m, h, w);
        ...
        if (checkBoundary(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, h, w)) {
            ...
            for (unsigned long i = up_corner_i; i <= bot_corner_i; i++) {
                ...
                for (int j = 0; j <= lw; j++) {
                    ...
                    ttrr = getIndexOfMax(mval, xval, yval, epsilon, generator, distribution);

                    idx = getIndicesT(coordTriangle_this_i, coordTriangle_this_j, up_corner_i,
                                      up_corner_j, bot_corner_i, bot_corner_j, m, h, w);
                    ...
                    if ((coordTriangle_this_i == (h - 1)) & (coordTriangle_this_j == (w - 1))) {
                        ...
                        max_of_3 = getMaxOfThree(mval, xval, yval, epsilon);
    ...
    for (int lev = depth; lev > 0; lev--) {
        setIndicesT(up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, lev, h, w);
        idx = getIndicesT(id1, id2, up_corner_i, up_corner_j, bot_corner_i, bot_corner_j, lev, h, w);
    ...
    tracebackPath_ = traceback_path;

    buildMSA(node, traceback_path);
...
std::vector<std::string> pPIP::getSeqnames(bpp::Node *node) {
    return sequenceNames_.at(node->getId());
}
